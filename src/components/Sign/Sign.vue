<template>
  <div class="display:inline">
    ...
    <v-dialog v-model="alertModal" max-width="300px" v-if="nowLevel == '2' && user !=='' && user !== undefined ">
       ...
       for(var i in chats) {
         if ( chats[i].userId == this.user ) {
           if ( !chats[i].chatting[chats[i].chatting.length-1].isReadUser ) {
             var msg = "읽지 않은 채팅방이 있습니다. =>"+chats[i].recruitPK;
              this.unread_alertlist.push({check:false,message: msg, url : chats[i].link, user : chats[i].companyId})
           }
         }
       }
     });
