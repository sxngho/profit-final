      ...
      >
        ...
        <v-layout wrap row justify-center style="margin-bottom:20px;">
          <v-flex xs12>
            <h1 class="text-center">
              <span class="fontHannaAir">공고 리스트</span>
            </h1>
          </v-flex>
          ...
          <v-flex xs10 class="text-center">
            <div class="text-center">
              <span
                class="fontHannaAir"
              >총 {{recruits.length}}개의 공고 중에 {{myRecruits.length}}개의 공고 지원가능합니다.</span>
            </div>

            <div>
              <fieldset
                style="padding:10px; border:2px solid lightgrey; border-radius:15px; "
                justify-center
              >
                ...
                >
                  <v-chip
                    v-for="item in mySkills[0].userSkills"
                    text-color="white"
                  ...
                  <span class="small grey--text">
                    <span class="fontHannaAir">
                      * 등록된 기술이 없습니다 *
                      ...
                      <br />
                      <br />기업들이 올린 공고를 보기 위해서는
                      <br />해당
                      <span class="fontHannaAir" style="font-weight: bold;">공고가 요구하는 최소 기술들</span>을 가지고 있어야 합니다.
                    </span>
                  </span>
                </v-layout>
              </fieldset>
        ...
        <v-layout wrap row>
          <v-flex
            ...
            sm6
            md4
            v-for="recruit in myRecruits"
            v-if="Filtering(recruit.data.requiredSkills) && !isEmpty"
            style="padding:10px; position:relative"
          >
            <img
              src="../assets/pin2.png"
              v-if="recruit.flag"
              style="width: 86px; position: absolute; z-index: 2;    left: -30px;
    top: -35px"
            />
            <RecruitDetail
          ...
          <v-flex v-if="isEmpty && mySkills[0].userSkills > 0" style="margin:20px;">
            <v-layout row wrap justify-center>
              <div style="display:inline;" class="text-center">
                <span class="small grey--text">
                  <span class="fontHannaAir">*지원 할 수 있는 공고가 존재하지 않습니다.*</span>
                </span>
                <br />
                <v-btn
                  :to="{ name: 'story', params: { id: user }}"
                  ...
                  color="success"
                >
                  <span class="fontHannaAir">프로젝트 작성하러가기</span>
                </v-btn>
              </div>
            </v-layout>
          </v-flex>
      ...
      >
        ...
        <v-layout wrap row>
          <v-flex xs12 sm6 md4 v-for="recruit in recruits" style="padding:10px;" v-if="!isEmpty">
            <RecruitDetail :recruit="recruit" :userdata="mySkills" :recruit_id="recruit.id" />
          </v-flex>
          ...
          <router-link to="/" class="bannerBox" style="text-decoration:none">
            <div class="bannerBtnBlack">
              ...
              <div class="studio_title titleBox">
                <h1>
                  <span class="fontStyle">홈페이지로 돌아가기</span>
                </h1>
              </div>
            </div>
          </router-link>
  ...
  methods: {
    ...
    async fetchData() {
      ...
      if (this.$store.getters.getLevel === 2) {
        ...
        for (var ii in this.mySkills[0].userSkills) {
          this.mySkills[0].userSkills[ii] = this.mySkills[0].userSkills[
            ii
          ].toUpperCase();
        }
        //모든 공고를 탐색하는데,
        for (var i in this.recruits) {
          ...
          if (!notIncludeExist) {
            ...
            this.myRecruits.push({
              id: this.recruits[i].id,
              data: this.recruits[i].data,
              flag: flag
            });
    ...
    dib(recruit_id) {
      if (!this.mySkills[0].dibs.includes(recruit_id)) {
        ...
        FirebaseService.UPDATE_userDibs(
          this.mySkills[0].dibs,
          this.$session.get("session_id")
        );
        this.$swal
          .fire({
            title: "찜목록에 추가되었습니다.",
            text:
              "작업실에서 확인이 가능하며 <br/> 채팅방에 들어가시면 지원이 완료됩니다!",
            type: "success",
          ...
          .then(result => {
            if (result.value) {
              ...
              this.$router.push({
                name: "story",
                params: { id: this.$session.get("session_id") }
              });
            }
          });
      } else {
        this.mySkills[0].dibs.splice(
          this.mySkills[0].dibs.indexOf(recruit_id),
          1
        );
        FirebaseService.UPDATE_userDibs(
          this.mySkills[0].dibs,
